...
def run_simulation(
    ...
    while heap:
        ...
        # -------------------
            ...
            if r.status == "waiting":
                ...
                if after_burnin(current_time):
                    abandoned_count += 1
                # IMPORTANT: do NOT delete here, because a driver may already be en-route.
                # We delete at pickup (when driver discovers cancellation) or at dropoff.
        ...
        # -------------------
            ...
            # ======= CRITICAL FIX: cancelled while en-route =======
            if r.status == "cancelled":
                # Driver spent time driving to pickup: count busy time (driving time)
                if d.busy_since is not None:
                    add_busy_time(driver_id, d.busy_since, current_time)

                # Driver also incurred petrol cost for deadhead miles, but earned no fare
                # Count this after burn-in (approximation: if pickup occurs after burn-in)
                if after_burnin(current_time):
                    deadhead_cost = cost_per_mile * r.pickup_dist
                    driver_earnings[driver_id] = driver_earnings.get(driver_id, 0.0) - deadhead_cost

                # Clear rider + driver state
                del rider_objects[rider_id]
                d.busy = False
                d.busy_since = None

                # If shift ended, go offline now
                if d.wants_offline:
                    record_online_time(driver_id, current_time)
                    del driver_objects[driver_id]
                else:
                    # Otherwise try match to next rider
